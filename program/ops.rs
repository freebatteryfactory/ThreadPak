...
//!
//! Signature declarations only, in valid Rust: each operation is declared as
//! a function-type alias (`pub type NameFn = fn(...) -> ...;`) stating one
//! lawful transformation — its exact inputs, its result, its refusal family,
//! and the bounds it consumes. The alias form keeps this file a compilable
//! declaration surface, so the dependency probe sees real compiler-visible
//! edges. Bodies land with the construction cuts (A3 for this owner); each
//! landing body is pinned to its declared signature by a Macroonz-generated
//! conformance assertion (`const _: DecideFn = decide;`). Nothing here
//! executes, and nothing claims implementation support.
//!
...
/// before allocation; refuses malformed or noncanonical bytes. The decoded
/// result carries the complete untrusted image record the later stages
/// judge; decode proves shape, never truth.
...
/// validation, consuming the decoded image — the decoded record it judges
/// rides inside `DecodedProgramImage`. One sealed transition of the affine
/// ladder; its refusal names the exact stage that failed.
...
/// is `KnowledgeRefusal::UndeclaredInformationLoss`. The landing body is
/// `#[must_use]`: the crossing evidence is the point.
