...
Selected facts consumed by the port owner's operations (`port/ops.rs`), passed in explicitly per `depot/README.md` ("Rows are passed, never fetched").
Every row carries binding time, change consequence, and status; recovered contradictions are preserved as separate rows, never averaged.
Machine-wide canon selections (digest family, byte order, domain-tag grammar) are the core canon owner's rows, cited here, never restated.
...
Nonclaims: a width proves no role; equal widths never substitute.
Falsifier: a value of one role accepted where another role is required because the bytes fit.
...
Posture classification: **safety-relevant** — absent a deployment row, the store refuses rather than defaulting open.
The raw-retention window and per-reason-class capture are deployment tunables under the four guardrails (recovered: "redacted-diagnostic-by-default … not a product decision").
...
Consumers: the two observation traits, `admit_wall_observation` and `admit_monotonic_observation` (every row above rides their `ClockSourceProfile` argument — swapping a row changes admission behavior with zero code edits), `validate_request` and `validate_response` (deadline domain and expiry evidence), the runtime deadline rebase.
Nonclaims: a profile row is a claim roster the harness qualifies — a compile is not a clock.
Falsifier: a cross-domain comparison, subtraction, or rebase without the explicit fallible conversion; an `Unstated` uncertainty admitted as a zero-width enclosure.
...
Falsifier: a claimed recovery route that does not exist before the irreversible Attempt; a carrier row treated as a support claim without its evidence row.
