...
- **Clock observation ports** — two separate role-specific contracts: wall observation and monotonic observation. One host adapter may implement both; the semantic contracts never merge, and there is no universal clock trait. Every observation is requested under a named domain **and** profile; a `ClockSourceProfile` declares its resolution, monotonicity claim, regression and suspend behavior, and uncertainty posture as facts the harness qualifies — a compile is not a clock. A raw observation carries its own evidence — the reading, the profile it was requested under, and the uncertainty the source reported — and this owner's admission (`admit_wall_observation`, `admit_monotonic_observation`) strengthens it into the admitted enclosure: earliest/latest bounds under the profile, a point observation being the degenerate enclosure only where the source explicitly claimed zero uncertainty (owner-ruled 2026-08-24). Unknown uncertainty is never zero uncertainty: `Unstated` resolves by the posture — widened to the profile's declared maximum, or refused (the paved posture is refusal). A raw observation is foreign material: only the admitted enclosure becomes accepted chronology through the chronology owner's admission, and only the admitted enclosure becomes deadline state through the runtime deadline owner's rebase — neither consumer ever receives a raw observation. A wall observation never impersonates a `CommitPoint`, an `AcceptedHlc`, an `AuthoritySequence` position, or any durable order.
...
1. **Observation reading shape — RULED by the repository owner, 2026-08-24.** The scalar-versus-interval framing was a false binary; the ruling composes both recovered receipts. A raw observation carries the reading **plus** the uncertainty the source reported; admission under the `ClockSourceProfile` produces the admitted enclosure (earliest/latest bounds), a point value being the degenerate enclosure only under an explicit zero-uncertainty claim. Unstated uncertainty resolves by the profile's posture — widened to the declared maximum, or refused — never silently zero. Chronology admission and the runtime deadline rebase consume only admitted enclosures, never raw observations. (`depot/port.md`, row port.wall-reading-representation records the resolution.)
