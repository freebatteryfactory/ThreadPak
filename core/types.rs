...
//! exact profile as an explicit argument; nothing here is ambient
//! (`depot/README.md`, "Rows are passed, never fetched").
//!
//! Cross-home names appear as bare names with owner attribution
//! (`ClockDomainId` — port); real imports arrive with the dependency probe.
//! Families whose law forces no type still declare nothing — their first
//! forced type arrives with its first consumer, per the no-prophecy rule.
...
/// A registered refusal is a semantic commitment: new meaning mints a new
/// identity, never reuses a retired one. Prose keyed by this identity adds
/// no variant and no condition.
...
/// How consuming a bound behaves. Affine only where duplicating the value
/// would fabricate capacity; a plain limit is copyable and needs no linear
/// ceremony (`ARCHITECTURE.md` §Bounds).
pub enum ChargeLaw {
    /// Charging consumes the value and returns a smaller successor; no
...
/// The settled default-posture classification every paved profile row
/// carries (D-DEFAULT-1; `depot/README.md` §Defaults are classified).
...
/// What changes when a row's value changes (`depot/README.md` §Binding time
/// and change consequence). A row without a declared consequence is invalid.
...
/// Identity of one registered unit. Role-distinct from every dimension
/// identity; no implicit conversion crosses units.
pub struct UnitId {
    /* registered id; width cites depot/core.md row registered-id-width */
}

/// One row of the bound-dimension register — the second level beneath
/// `BoundClass` (recovered from the archived register's row shape). The
/// numeric value and the owning-family attribution are the depot row's
/// facts per the depot row law; this type binds the typed facts that make
/// the value honest.
...
/// The shared affine budget mechanic. INTERNAL machinery: the public API is
/// each owner's named wrapper (`QueryWorkBudget`, `SemanticWorkBudget`,
/// `KnowledgeBudget`, `ReconciliationStepBudget`, `PumpWorkBudget` — the
/// five identical claimants that met this family's extraction bar). Charging
/// consumes the value and returns a smaller successor via `ops::charge`;
/// no widening method exists anywhere. Deliberately neither `Clone` nor
/// `Copy` — duplicating it would fabricate capacity.
///
/// `D` is the owner's dimension marker; the invariant phantom keeps one
/// dimension's budget from ever standing in for another's.
#[must_use]
pub struct Budget<D> {
    /// Magnitude substrate: u64, per depot/core.md row `budget-magnitude`
    /// (candidate; matches all five claimants).
    remaining: u64,
    dimension: PhantomData<fn(D) -> D>,
}

/// Typed exhaustion of one budget: which dimension, what was demanded, what
/// remained. A terminal typed fact, never a panic.
#[must_use]
...
/// A signed difference between two time values in one clock domain. A
/// negative delta is valid evidence — never clamped, never saturated to
/// zero, and deliberately not authority-capable: it is diagnostic evidence,
/// not a duration, deadline, order, or budget.
///
/// (`ClockDomainId` — port owner. The std `core::time::Duration` has no
/// signed counterpart; this role is the gap it cannot fill.)
pub struct TimeDelta {
    domain: ClockDomainId,
    /// Signed magnitude; substrate i128 per depot/core.md row
    /// `time-substrate` (candidate — matches `RawWallObservation.reported`).
    magnitude: i128,
}
...
/// never reused across versions, and numerically newer never implies
/// compatible.
...
/// field. Never silently recycled.
pub struct FieldId {
    /* closes with the identity profile; width cites depot/core.md */
}

/// Identity of one variant within one schema commitment. Same reuse law as
/// `FieldId`: a removed variant identity is never reused.
pub struct VariantId {
    /* closes with the identity profile; width cites depot/core.md */
}

/// The canonical-byte commitment a `SchemaId` derives from — the schema's
/// semantic meaning as domain-tagged canonical bytes, distinct from any
/// descriptor artifact's digest.
pub struct SchemaCommitment {
    digest: Digest,
}

/// The explicit unknown-member contract every schema declares — never
/// silent (recovered roster, ch10). `OpaquePreserved` is monotone
/// extendability (a FUTURE schema may interpret), not pending: there is no
/// future in which THIS schema determines an unknown member, so this never
/// collapses into `Truth::Pending`.
...
/// Witness that one byte sequence passed bounded schema validation under one
/// exact schema commitment and policy. Possession proves the check ran;
/// it proves nothing about domain truth.
#[must_use]
...
/// The typed refusal family of schema validation. Each variant traces to a
/// law sentence of the schema family; the roster closes with the schema
/// profile.
pub enum SchemaValidationRefusal {
    ...
    /// An unknown member arrived under `UnknownMemberPolicy::Closed` or an
    /// ununderstood `RequiredExtension`.
...
/// The settled six-class identity classification (roster recovered from the
/// archived matrix; variant spellings are draft, the classification law is
/// settled). Every identity in the machine falls into exactly one class,
/// and each class answers the layout and capability questions once —
/// per-class rules such as scope-guarded comparison and no derived `Ord`
/// on scoped values ride the class, never the call site.
pub enum IdentityClass {
    /// Equal meaning ⇒ equal id: domain-tagged digest of normalized
    /// meaning; no order; keyed under a `KeyScopeId` when protected.
    SemanticCommitment,
    /// A receipt over exact bytes; proves the byte role its preimage names
    /// and nothing more.
    ByteDigest,
...
/// and protected material are derived and destroyed. Declaration seat only:
/// key custody, installation, and shredding belong to the owners that hold
/// keys (view resolution, port quarantine, host installation), never to
/// this family.
...
/// domain-separation tag, so every identity names its family by
/// construction and no ambient "which algorithm?" question exists.
/// Cross-family comparison refuses at the scope guard; migration between
/// families is a named re-digest morphism, never a reinterpretation.
pub struct DigestFamilyId {
    /* registered id; width cites depot/core.md row registered-id-width */
}

/// One digest-family selection: which family, under which realization
/// profile row. Realizations of one family (native dispatch, portable
/// Rust, wasm) must produce identical outputs against the depot's golden
/// vectors — swapping realizations is requalification, never a new
/// identity; swapping families is a new canonical profile.
...
/// Registered identity of one tag role (the `<role>` segment) — also the
/// wire-role projection's id.
...
/// under that family's declared bound. The preimage is the identity
/// substrate: the identity derives from it — never the reverse — and
/// binding a preimage supports an integrity or identity claim without
/// independently proving the underlying assertion true.
pub struct CanonicalPreimage {
    /* family binding plus bounded bytes; composition is per-commitment-
     * family law, declared with each owner's commitment row */
}
...
/// One digest value: a receipt over the exact bytes of one preimage under
/// one domain tag. Never authority over meaning, never ordered, never
...
/// The derivation-context projection of one `DomainTag` — the exact
/// context string keyed derivation consumes. Generated from the register;
/// hand-authored contexts are invalid.
...
pub struct CanonProfile {
    ...
    /* byte-order, integer-posture, text-form, and framing selections are
     * the depot/core.md rows this profile binds */
}
