...
A refusal family takes one of three body shapes, chosen by how its checks relate — each shape exists to kill a named lie. The shape is chosen per family at its owner; this family owns the shape vocabulary and the anatomy law, and deliberately declares no universal refusal type. The roster, recovered whole from the banked authoring census (`depot/core.md` §Refusal authoring shapes): **SingleCause** — exactly one mutually exclusive cause, or an explicit deterministic cause-selection rule; realized as an enum. **IssueCollection** — a bounded, nonempty, canonical collection of independently established issues, carrying its completion or early-stop posture, with no invented primary issue; realized as a one-field struct. **InseparablePair** — two facts jointly constituting one refusal, neither an optional secondary cause; realized as a two-field struct. This is ThreadPak authoring law: generation machinery consumes the classification, and no public runtime shape value ever flows through the machine.
...
The settled identity-class matrix (six classes, with per-class capability rules such as scope-guarded comparison and no derived `Ord` on scoped values) is law. The identity law is two-column (restored 2026-08-24): identity class says what kind of identity, and `IdentityCreationLaw` says how the family mints it — derived commitment, fresh opaque, or authority-assigned. The class never implies the creation law: `TurnId` is a derived occurrence, a fresh opaque id is a fresh one, and every family's register row carries both columns. The algebra is declared here — `IdentityClass`, `IdentityCreationLaw`, `DigestProfile`, `IdentityProfile`, `KeyScopeId` in `types.rs` — and its selected values (per-class widths, byte order, digest family, text form, per-family creation law) are the `depot/core.md` rows, ratified by the canon-packet mint (2026-08-24) or carried per row status. Every width in every home cites its row; no owner hard-codes one independently. Keyed derivation is scope-bound: `KeyScopeId` is declared here as a seat only — key custody, rotation, and destruction belong to the owners that hold keys.

Key-custody law (closed 2026-08-24): owned key material is never `Clone`, never serialized, never formatted or logged; copies are minimized; scope and generation are explicit; destroying a scope's key revokes the relevant live authority, and a later protected resolution over a shredded scope reports the shredded outcome honestly — never a fabricated success. Zeroing one buffer never claims every historical copy vanished: zeroization is defense-in-depth, never proof of global erasure. The physical mechanism (zeroizing storage and drop behavior for in-process key bytes; an opaque handle with a backend destruction contract for external custody) is selected per key-custody realization row at the manifest pass.
...
Shared time value roles are declared here: a signed `TimeDelta` is not an unsigned span, and neither is a deadline, a chronology value, or an observation. A negative delta is valid evidence, never clamped. The generic unsigned span is `core::time::Duration` (owner ruling 2026-08-24, stdlib-batteries applied): a type is not contaminated by a method (`saturating_sub`) our semantic operations never call — ThreadPak semantic code uses only checked operations on it, and owner-specific `Time`-class wrappers (horizons, ceilings, allowances) keep their roles at their owners. No custom generic span type exists.
...
5. **Identity profile.** Fact: exact widths, preimage grammar, the six-class roster, and the per-family creation law. Owner: the identity family — the algebra in `types.rs`, the selected values as `depot/core.md` rows. Operation: this contract's declarations plus the owner's row acts. Carrier: every other home cites its row, never mirrors it. Refusal: no other file restates the matrix or hard-codes a width. Chronology: carries the settled identity-matrix rulings, the owner's seed ruling, and the canon-packet mint (all 2026-08-24); remaining candidate/withheld rows stay per row status, machining is owner-derived.
...
1. **Withheld depot rows stay withheld.** The `depot/core.md` withheld rows (public-digest entropy floor, KDF parameters, Unicode identifier pin, numeric authority families, schema shape axes) record unmade decisions; each closes with its consumer or an owner statement.
2. **Key-custody mechanism.** The semantic custody law is closed (§3); the physical mechanism selection (zeroizing storage / `zeroize` feature composition per realization profile) lands with the manifest pass — recorded as the `zeroize-mechanism` withheld row.
