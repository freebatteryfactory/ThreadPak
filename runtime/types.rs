...
//!
//! Declarations only. Operations are declared in `ops.rs` beside this file;
//! no behavior is defined here. Crossing types named in field positions —
//! `EventProposals`, `EffectProposal`, `ProgramImageId`, `ImmediateResult`,
...
//! the probe exists.
//!
//! Every field here is private, and every field is a correctness-bearing
//! relationship the owner contract promises. Where a leaf representation is
//! written `/* closes with … */`, the relationship is law and only the exact
//! byte shape remains owner-derived machining.
//!
...
//! or any serialization. Live custody types must also never become `Send` or
//! `Sync` by accident: an auto-derived crossing would carry live authority
//! between workers without an admission decision, so custody interiors are
//! chosen at the guard pass to make the wrong crossing unrepresentable.
...
/// Identity of one logical operation — the application-meaningful unit one or
/// more Turns realize. One of the four never-substitutable lineage identities
/// (logical operation, Turn, EffectIntent, Attempt).
pub struct LogicalOperationId {
    /* closes with the identity profile */
}

/// Identity of one logical transition. Derived from the Turn's
/// identity-bearing inputs; replay reconstructs the same Turn where lawful,
/// and a changed identity input is a different Turn. The exact preimage
/// roster and width close with the identity profile; the recovered preimage
/// candidates are depot rows, not code.
pub struct TurnId {
...
/// Identity of one delivery relationship this owner drives. Role-distinct
/// from `ProcessId` and from the view owner's `SubscriptionId`.
pub struct DeliveryId {
    /* closes with the identity profile */
}

/// The durable admitted state of one logical process — the state Stitch
/// consumes and succeeds. Recovery reconstructs it from accepted history
/// selected by the process's input contract plus its durable checkpoint;
/// queues, channels, and notifications are acceleration, never this state.
pub struct ProcessState {
    /* owner-derived durable representation */
}

/// The declared contract of one logical process: its input contract (which
/// accepted work it consumes), its deferred-input bounds, its ordering
/// relationship, and its no-progress disposition. Declared once per process
/// role; numeric values live in the depot.
pub struct ProcessContract {
    ...
    /* input contract, ordering relationship, and no-progress disposition
...
/// The frozen input binding of one Turn: the exact source set and one exact
/// Cut per participating source — never a generic frontier, HLC, page
/// cursor, route, delivery sequence, or wall-clock instant. A missing source
/// stays explicit and is never silently replaced by the newest reachable one.
pub struct FrozenTurnInputs {
    sources: SourceSet,
    /* one exact Cut per source, in canonical participant order, plus the
     * input identity commitment; closes with the canon profile */
}

/// The declared semantic bounds one Turn runs under. The bound set's members
/// are owner-named types classified per root law; numeric values live in the
/// depot.
pub struct DeclaredTurnBounds {
    /* owner-named bound roster; closes with the process-contract seam */
}

/// Which checkpoint subject this Turn feeds and what prerequisite evidence
/// it contributes toward that subject's next lawful advance. A Turn that
/// feeds no checkpoint chain says so explicitly; nothing infers a
/// consequence from silence.
pub struct CheckpointConsequence {
    subject: CheckpointSubject,
    /* contributed prerequisite evidence; decomposition closes with the
     * checkpoint contract */
}
...
/// violated law, the typed owner, the offending value's role, and the
/// repair direction when realized.
#[must_use]
pub enum StitchRefusal {
    /// The stimulus is bound to a prior process generation and is not
    /// runnable work in this one.
    StaleGeneration,
...
/// the retained stimulus, why it is deferred, the exact state or evidence
/// that could unblock it, its absolute deadline, and its collation position.
/// Reconsidering it under unchanged state and unchanged evidence is no
/// progress, and the runtime recognizes no progress rather than spinning.
...
/// Why one admitted input cannot run now. Names the blocking fact, never a
/// generic "pending".
pub struct DeferralReason {
    /* blocking-fact binding; closes with the stimulus-algebra seam */
}

/// The exact state or evidence that could make one deferred input
/// admissible. Inert data; it performs nothing.
...
/// observer abandonment, deadline expiry, close, drain, and shutdown. The
/// authorizing grant is role-specific and closes with the guard pass.
pub struct CancellationRequest {
    target: CancellationTarget,
    /* authorizing-grant binding; closes with the guard pass */
}

/// What one cancellation request targets. (Draft spelling; the two arms are
/// law — Bvisor cancels physical Attempts, the runtime cancels logical
/// operations.)
...
/// Live custody of one runtime lane. One drive call owns one lane;
/// same-lane reentrancy is unavailable by default. Never `Clone`, never
/// serialized, never accidentally `Send`/`Sync`.
#[must_use]
...
/// Identity of one durably admitted external intent. One of the four
/// never-substitutable lineage identities; minted only by REQUEST/PEND
/// admission.
pub struct EffectIntentId {
    /* closes with the identity profile */
}

/// Which admission road minted one durable intent. REQUEST returns without
/// waiting; PEND additionally drives one immediate bounded Attempt. The
/// posture never changes the record's meaning or survival.
pub enum EffectAdmissionPosture {
    Request,
    Pend,
}

/// The durable, runtime-owned record of one admitted external intent —
/// minted by REQUEST or PEND admission consuming a program-produced
/// `EffectProposal`. Append-only fact: it survives any later semantic
/// refusal, and no later evidence edits it. Effect *meaning* stays
/// program-declared; this record owns admission, custody, and recovery
/// accounting.
...
/// The identity under which duplication of one intent is recognized, per
/// the idempotency ladder: natural business identity, reservation token,
/// generated per-call key, or explicit key. Content-derived, wall-clock,
/// session, route, connection, host, shard, and Attempt identities are all
/// unlawful substitutes.
pub struct EffectIdempotencyIdentity {
    /* ladder-rung binding; closes with the identity profile */
}

/// The retry-discharging evidence that one proposal was durably admitted as
/// one intent. Claims exactly the admission boundary — never physical
/// execution, outcome, or checkpoint progress.
#[must_use]
...
pub enum EffectAdmissionRefusal {
    /// The proposal's recovery posture claims a route that is not declared
    /// before the irreversible Attempt.
...
/// carries the establishing conclusion; `OutcomeUnknown` is a stable honest
/// result, never relabeled as failure. (Draft spelling on the payload.)
pub enum OutcomeKnowledge {
    /// Admitted evidence establishes the outcome; the payload binds the
    /// establishing reconciliation conclusion.
    Known(EstablishedOutcome),
    /// An external consequence may exist; evidence cannot establish it.
    OutcomeUnknown,
}

/// The established external conclusion of one effect, bound to the evidence
/// that establishes it. A claim of knowledge never travels without its
/// establishing basis.
pub struct EstablishedOutcome {
    intent: EffectIntentId,
    /* establishing evidence binding; closes with the reconciliation
     * contract */
}

/// Whether reconciliation is owed for one intent. Carrying the disposition
/// inside `Complete` makes a disposition-without-completion unrepresentable,
/// and `Outstanding` can never masquerade as a resolved handling.
...
/// How one completed reconciliation concluded — meaning available only at
/// completion. The lifecycle answers *whether*; this answers *how*.
...
/// terminal outcome knowledge: the intent, its known Attempts, the
/// append-only evidence so far, and the applicable recovery posture.
pub struct OutstandingEffect {
    intent: EffectIntentId,
    /* bounded known-Attempt roster, append-only evidence references
     * (sealed AttemptReports, acknowledgements, outcome-query evidence),
     * and the recovery-contract binding */
}

/// The durable fact that outcome knowledge for one intent remains
...
/// append-only evidence. Never rewrites an earlier Attempt or observation;
/// later evidence supersedes the conclusion while the record stands.
#[must_use]
...
/// The closed roster of lawful next actions one reconciliation may
/// conclude. A physical inconvenience never adds a variant.
...
pub enum ReconciliationRefusal {
    /// The concluded action lies outside the operation's declared recovery
    /// contract.
...
/// Maximum retained age of one deferred input. A Time-class bound; the
/// numeric value lives in the depot.
...
/// death; never process-monotonic by itself. Exactly one policy governs one
/// operation across admission, execution, suspension, retry, outcome query,
/// reconciliation, and every host adapter. (Shape roster recovered; the
/// paved-default classification is a depot row.)
pub enum DeadlinePolicy {
    ...
    /// Wall-anchored with declared tolerance — expressible, explicit, never
    /// a default.
...
/// enforcement state; a policy has no "remainder" — the remaining allowance
/// is derived at rebase.
...
pub struct LiveDeadline {
    /// The one monotonic clock domain this state is meaningful in
    /// (port-owned role).
    domain: ClockDomainId,
    /* private expiry point */
}

/// Closed refusal family for deadline rebase and checks. Lost provenance
/// defaults to refusal — the safety-relevant posture.
#[must_use]
pub enum DeadlineRefusal {
    /// Spent-allowance provenance is lost or incomplete; refusal, never an
    /// optimistic rebase.
    LostProvenance,
...
/// The role-branded subject one checkpoint chain is keyed by. A process
/// checkpoint never substitutes for a subscription or delivery checkpoint
/// merely because the identities share a width.
...
/// Generation of one checkpoint subject, for wrong-generation refusal at
/// admission.
...
/// The expected predecessor of one proposed advance: the prior accepted
/// advance, or the explicit genesis posture of a fresh chain.
pub struct ExpectedCheckpointPredecessor {
    /* predecessor binding or genesis posture; closes with the checkpoint
     * contract */
}

/// A proposed checkpoint advance. Admissible only when every skip
/// prerequisite exists; advancing before prerequisites is unrepresentable
/// or refuses.
#[must_use]
...
/// The exact domain Cuts whose completed work one advance covers. References
/// domain regions; never inhabits or mutates them.
...
pub struct RequiredReceiptSet {
    /* role-specific receipt references; closes with the checkpoint
     * contract */
}

/// Every outstanding effect and reconciliation obligation accounted by one
/// advance. A forgotten obligation makes the proposal inadmissible.
...
/// Maximum branches of one join. Work class.
...
/// ambient capability, or serialized live authority. The exact
/// representation and operator inventory remain a recorded open seam.
pub struct VmValue {
...
/// recovery mints a fresh Attempt. The persisted continuation record carries
/// the `DeadlinePolicy` reference plus `ConsumedBudgetEvidence` — never a
/// live monotonic deadline. Never `Clone`, never serialized live.
#[must_use]
pub struct SuspendedVm {
    request: PortRequestId,
    attempt: AttemptId,
    generation: ProcessGeneration,
    /* private: resume coordinate, bounded captures, deadline-policy and
     * consumed-evidence references, cancellation posture */
}

/// Closed refusal family for execution-state integrity. Semantic image
/// validation is the program owner's and is not represented here.
#[must_use]
...
/// The lineage relationship of one Attempt: edge references by identity,
/// never containment. Cancellation and termination propagate along typed
/// fate-links, not ownership.
...
pub enum PhysicalAdmissionRefusal {
    /// A required reservation dimension is unavailable; everything acquired
    /// was released.
...
/// never serialized, never accidentally `Send`/`Sync`; a persisted
/// description never resurrects custody.
#[must_use]
...
/// or termination. Same custody laws as `AdmittedAttempt`.
#[must_use]
...
pub enum AttemptResponseRefusal {
    /// A correctness-bearing coordinate (request, Attempt, role, generation,
    /// deadline) does not match.
    WrongCoordinates,
    /// The one-shot authority was already consumed.
    AuthorityConsumed,
    /// The target Attempt is no longer live; the response may remain
    /// physical evidence.
    AttemptNoLongerLive,
    /* the roster closes with the admission contract */
}
...
/// the operation's `DeadlinePolicy` at admission, meaningful in one clock
/// domain; no adapter, retry, or fresh Attempt mints a later one.
pub struct AbsoluteDeadline {
    id: AbsoluteDeadlineId,
    /// The clock domain the expiry commitment is meaningful in (port-owned
    /// role).
    domain: ClockDomainId,
...
/// The declared algebra of one driver profile: who drives, when driving is
/// required, the pump work allowance, runnable-lane fairness posture,
/// starvation posture, wake registration and lost-wakeup prevention,
/// spurious-wake and coalescing handling, reentrancy posture, callback
/// custody, close/drain/shutdown/restart selections, panic containment
/// selection, and which liveness claim becomes unavailable when the host
/// stops driving. Fairness is a declared claim, never queue-library
/// folklore.
pub struct DriverProfile {
    /* declared axes above; exact representation closes with the paved
     * driver escalation */
}

/// The declared algebra of one checkpoint storage profile: the publication
/// family binding for accepted advances and the fast-start posture. The
/// storage mechanism is realization; this algebra is not.
pub struct CheckpointStorageProfile {
    /* declared axes; closes with the checkpoint-storage seam */
}

/// The declared algebra of one reconciliation profile: outcome-query
/// cadence posture, compensation authorization posture, and partial-outcome
/// admission posture — each selecting within the operation's recovery
...
/// roster (caught unwind as physical observation, abort-and-fresh-Attempt,
/// isolated worker) is the recorded owner escalation; this type seats the
/// algebra without closing the fork.
