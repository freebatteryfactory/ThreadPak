...
//! role carries; behavior lives in this owner's thin operations (`ops.rs`).
//! Every derived resident is rebuildable from accepted history at exact Cuts;
//! the read-side authority roles declared here (`ReadGrant`,
//! `ProtectedResolutionGrant`) are declaration seats only — no view operation
...
//! by wrapping this owner's `SubscriptionId` in its role-branded
//! `CheckpointSubject`; no runtime type is referenced here.
//!
//! Profiles declared near the end of this file are the owner's configuration
//! algebra: operations receive them as explicit arguments, selected values
//! live in `depot/view.md`, and nothing here is fetched ambiently
//! (`depot/README.md`, "Rows are passed, never fetched").
//!
//! Declarations only. Fields appear where product law pins them; interiors
//! marked as seams close with their named pass (identity profile, canon
...
/// Minted only by the equality-proving operation; possession is the only
/// lawful road past `SelectionRefusal::RowDomainEqualityUnproven`. Equal
/// cardinality mints nothing.
pub struct RowDomainEqualityWitness {
    /* the two proven-equal domain identities and the proving evidence;
     * construction closes with the guard pass */
}
...
/// A query names its source set, its frame expectations, its Cut
/// requirements, and its bounds; every read names what "now" means.
pub struct Query {
    sources: SourceSetId,
    frame: ReferenceFrameId,
    frame_version: FrameVersion,
    /* question body (application-typed), Cut requirements, and the bound
     * set; the body's exact shape closes beside Fix's public shape
     * (Escalation 1 in README.md) */
}

/// Which road produced a derived answer. Parity law: at the same claim,
/// source set, exact Cuts, frame, relation versions, configuration, and
/// profile, the two roads must agree — and on disagreement the maintained
/// road loses. Provenance is honesty about the road taken, never permission
/// to skip parity.
...
/// Declared portable work consumed by one advance or resolve: a function of
/// the affected input set, never CPU cycles, wall time, or scheduler
/// observations (rail 11).
pub struct ConsumedDerivationWork {
    /* per-dimension accounting; the dimension roster closes with the
     * work-profile rows in depot/view.md */
}

/// Progressive-explanation bindings of one evaluation (rail 14): concise
/// description, typed signature, structured explanation, complete
/// definitional expansion — four readings of one evaluation, never four
/// evaluations.
...
/// `README.md`); the axes and relationships below are law.
pub struct Fix<T> {
    value: T,
    sources: SourceSetId,
    at: FederationCut,
...
pub struct UnobservedSources {
    /* named missing participants; representation closes with the identity
     * profile */
}
...
/// substitute a newer or older result — an exact read that cannot be served
/// at its requested Cut refuses; it never silently serves another Cut.
pub enum Freshness {
    /// The bound Cuts are exactly the requested Cuts.
    AtRequested,
    /// The bound Cuts lag the requested posture; the lag is named evidence.
    BehindRequested {
        /* the requested-versus-bound Cut relationship; representation
         * closes with the guard pass */
    },
}
...
/// declared work it consumed. Producing this value publishes nothing and
...
/// state. It binds the exact operation it continues — changing any bound
/// fact is an incompatible continuation, never a silent rebase.
pub struct Cursor {
    at: FederationCut,
    /* continuation bindings: operation identity, source lineage and
     * generations, selectors and filters, declared order and tie-breaks,
     * direction, page and work bounds; exact roster and encoding close with
     * the canon profile (recovered thirteen-fact binding set is banked in
     * depot/view.md) */
}
...
pub struct TemporalClaim {
    ...
    /* claim body (the temporal proposition itself); closes with the
     * temporal starter family (Escalation 3) */
}
...
pub struct TemporalMonitorState {
    ...
    /* bounded state body; representation closes with the monitor
...
pub struct SelectionMask {
    ...
    /* logical length (padding bits never enter membership or iteration —
     * width per the canon profile; the recovered sketch and the count-width
     * law disagree, preserved in depot/view.md), deterministic iteration
     * order, and the private membership representation */
}

/// A set of candidate members proposed by a qualified approximate mechanism.
/// Candidates never establish truth, absence, order, completeness, or
/// authority; the only lawful consumers are exact verification against
/// accepted history or an honest incomplete result. No conversion to
/// `SelectionMask` or to any result type exists.
...
pub struct ReadGrant {
    /* scope, attenuation, generation, expiry bindings; close with the guard
     * pass */
}
...
pub struct ProtectedResolutionGrant {
    /* key-scope, extent-scope, generation bindings; close with the guard
     * pass */
}

/// The result of one protected resolution. Role named here; the exact result
/// family (what a resolution may report besides the resolved extent) is
/// owner-derived work that closes with its construction cut (README
/// Escalations).
pub struct ProtectedResolution {
    /* resolved extents under the applicable grant; family closes with its
     * cut */
}
...
/// Evidence that the maintained road and the reference road agreed at one
/// exact claim, source set, Cuts, frame, and profile. Evidence about one
/// comparison — never a standing certificate.
pub struct ParityWitness {
    /* the compared claim binding and both roads' result commitments */
}

/// Evidence that the two roads diverged. The maintained result loses: it is
/// discarded and rebuilt from accepted history. Divergence evidence is
/// never silently dropped.
pub struct ParityDivergence {
    /* the compared claim binding and the differing result commitments */
}

/// The outcome of one parity judgment. Judging is an operation; an
/// incomparable pairing (different claim, sources, Cut, frame, or profile)
/// refuses as `QueryRefusal::ParityIncomparable` instead of producing this.
#[must_use]
...
/// separate operations; the standing typestate roster for those stages is a
/// realization seam under this contract.
pub struct MaterializationGeneration {
    role: MaterializationId,
    generation: MaterializationGenerationId,
    applied: AppliedCut,
    /* derivation-profile binding and incorporation evidence; the evidence
     * shape closes with the qualification pass */
}
...
pub struct DataBlock {
    ...
    block: DataBlockId,
    row_domain: RowDomainId,
    /* column/extent descriptor bindings (grouping lawful, omission not) and
     * the descriptor's own decode/allocation/component/row/result bounds —
     * a reader refuses before allocating; bound values are
     * MaterializationProfile rows in depot/view.md */
}
...
/// Result-class limit: maximum rows one query may return. A row limit binds
/// discovery before decode, join, sort, or materialization — never
/// read-everything-then-return-the-first-N — and a full page proves no
/// completeness.
...
/// The dimension a temporal horizon is measured in. The two dimensions never
/// substitute or convert.
...
pub struct ViewAdvanceProfile {
    /* declared per-event work formula binding and advance bounds; rows in
     * depot/view.md */
}

/// The selected bounds of one pull-lane resolve: row ceiling, traversal
/// depth, fan-out, and the work-budget denomination budgets are minted from.
...
/// The selected bounds of one temporal monitor: state-size ceiling and
/// advance work posture. Fast-start posture is a recorded owner escalation,
/// not a profile row.
pub struct MonitorProfile {
    /* monitor state and work bounds; rows in depot/view.md */
}

/// The selected bounds and mechanism postures of one materialization role:
/// byte ceilings and the descriptor's decode/allocation/component/row/result
/// bound values.
pub struct MaterializationProfile {
    bytes: MaterializationByteLimit,
    /* descriptor bound values and layout-mechanism selection; rows in
     * depot/view.md */
}

/// Qualified physical representations one semantic mask may take (recovered
/// mechanism roster; qualification rows in depot/view.md). Conversion among
/// qualified representations is a mask operation; membership meaning never
/// varies by representation, and physical padding never enters membership or
/// iteration.
...
pub enum QueryRefusal {
    ...
    /// A closure-requiring conclusion (absence, negation, exhaustive search,
    /// final order, top-k) was demanded over an unclosed source set.
    ...
    /// A parity judgment was requested over an incomparable pairing —
    /// different claim, source set, Cut, frame, or profile.
    ParityIncomparable {
...
pub enum AdvanceRefusal {
    ...
    /// The delta does not abut the state's `AppliedCut` — advancing over a
    /// gap would silently skip admitted history.
    DeltaNotContiguous {
        /* the state's AppliedCut versus the delta's boundary */
    },
    /// The advance exceeded its declared per-event work or an admitted
    /// bound.
    BoundExceeded {
...
pub enum MonitorRefusal {
    ...
    /// Settlement was demanded where the horizon cannot close over the
    /// claim's source set; the monitor reports incompleteness instead of
    /// settling.
    ClosureUnavailable {
...
pub enum SelectionRefusal {
    ...
    /// A conversion targeted a representation not qualified for this
    /// domain's profile.
    RepresentationUnqualified {
        /* the requested representation */
    },
    /// The selection's reported cardinality exceeded its Result-class
    /// limit.
    CardinalityExceeded {
...
pub enum MaterializationRefusal {
    /// A generation claims incorporation its evidence cannot prove — new
    /// physical bytes masquerading as a newer `AppliedCut`.
    AppliedCutUnproven {
        /* the claimed AppliedCut and the missing evidence */
    },
    /// A stored occurrence failed integrity; it is discarded and rebuilt,
    /// never reported as absence and never served as history.
    OccurrenceCorrupt {
        /* the failing occurrence identity */
    },
    /// A lifecycle stage was invoked out of order (publish before validate,
    /// activate before publish, reclaim with live readers).
...
pub enum ReleaseRefusal {
    ...
    GrantAbsent,
    /// The request exceeds the installed grant's scope; a derived result
    /// never widens the grant that produced it.
    GrantScopeExceeded {
        /* the requested extent versus the grant's scope */
    },
    /// The grant's generation is no longer current.
    GrantGenerationStale,
    /// Resolution was requested ahead of the release chain's order —
    /// authorization and exact selection come first; decrypt last, decrypt
    /// least.
...
pub struct AdmittedDelta<'history> {
    ...
    /* delta bounds and the contiguity binding to the prior AppliedCut;
     * close with the advance contract */
}
