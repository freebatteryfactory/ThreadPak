...
//!
//! Declaration form: each operation is a function-type alias
//! (`pub type NameFn = fn(...) -> ...;`), so this file is valid Rust and the
//! dependency probe sees compiler-visible edges. No bodies — bodies land with
//! their construction cuts, each pinned to its declared signature by a
//! Macroonz-generated conformance assertion
//! (`const _: AdmitEventFn = admit_event;`). Nothing in this file executes,
//! and nothing here claims implementation support.
//!
...
//!
//! Foreign roles by owner: `WallObservation` (port — the admitted observation
//! enclosure produced by the port owner's `admit_wall_observation` under a
//! declared `ClockSourceProfile`; raw readings never reach this owner).
//! Storage physically realizes publication through the storage port family
//! (`StorageOperation` roster; contract declared in the port grammar,
//! projected as a depot row).

// ---------------------------------------------------------------------------
// Accepted history
// ---------------------------------------------------------------------------

/// Operation `admit_event` — admit one proposed fact into accepted history:
/// the single domain-fact admission in all of ThreadPak.
///
...
/// Operation `relate_federation_cuts` — relate two federation cuts over
...
/// Operation `admit_chronology` — pure chronology admission: policy + prior
/// state + one admitted wall-observation enclosure + optional source
/// chronology → the successor state and an `AcceptedHlc`, or a typed refusal.
///
/// The observation is the port owner's admitted enclosure (`WallObservation`:
/// earliest and latest bounds under a declared `ClockSourceProfile`) — never
/// a raw reading. A point reading is the degenerate enclosure only when the
/// source explicitly claims zero uncertainty; unstated uncertainty was
/// already resolved at the port boundary by the profile's posture — declared
/// maximum or refusal, never zero.
///
...
/// Operation `merge_chronology_summaries` — merge two chronology summaries:
/// pure, same-profile, componentwise maximum. Total over validated
/// same-profile summaries; consults no wall clock, evaluates no trust,
/// stamps no event, claims no durable progress.
...
/// Operation `reserve_ingress` — idempotent `Reserve`: mint or recover the
/// reservation for one client nonce and intent commitment.
///
...
/// Operation `admit_claim` — admit one validated foreign claim into durable
/// ingress custody: the ClaimFirst terminal boundary.
///
...
/// Operation `resolve_claim` — record the outcome of one admitted claim's
...
/// Operation `mint_domain_admission_receipt` — mint the DomainFirst terminal
/// receipt after the accepted event's publication boundary is durable: the
/// binding of this exact submission's idempotency identity to the accepted
/// identity and its commit point.
...
/// Operation `recover_prefix` — pure recovery classification over bounded
/// scan evidence: establish the exact recovered boundary and classify
...
/// Operation `seal_region` — seal one region's writable epoch at an exact
/// Cut. A sealed parent accepts no later write.
///
/// Consumes: a grant scoped to `PartitionOperation::Seal`, the region and
/// epoch, the region's current accepted boundary, and the caller's claimed
/// seal boundary. The claimed boundary must be the current accepted
/// boundary — sealing behind it would orphan accepted material; sealing
/// ahead of it would seal history that does not exist. Refuses:
/// `WrongAuthority`, `StaleSealCut` (the claimed boundary is not the current
/// accepted boundary). `ParentNotSealed` preconditions of downstream steps
...
/// Operation `compute_split_witness` — prove one lawful split of a sealed
/// parent: children pairwise disjoint, union exactly the parent, no gap, no
/// overlap.
///
/// A pure geometric proof: evidence in, witness out, no authority consumed.
/// Child identities alone can prove nothing about coverage — every child
/// carries its declared geometry, judged against the parent's declared
/// geometry. Refuses: `ParentNotSealed`, `ChildrenOverlap`, `CoverageGap`.
...
/// Operation `compute_cut_succession` — translate a parent position into a
/// successor scope: the only lawful bridge between parent and child cuts.
///
/// A pure proof: it consumes succession evidence, not authority — the split
/// witness is the relation that makes the translation lawful. Refuses:
/// `SuccessionEvidenceMissing` (the successor is not a proven child of this
/// exact sealed parent). `WrongAuthority` belongs to the admission
/// operations, never to this proof.
...
/// Operation `activate_epoch` — activate one fresh epoch on a successor
/// region. Activation precedes routing publication. Consumes a grant scoped
/// to `PartitionOperation::ActivateSuccessor`. Refuses: `WrongAuthority`,
/// `ParentNotSealed`.
...
/// Operation `publish_routing` — publish routing for a succession: last,
/// after every activation. Routing reports authority and never grants it.
...
/// Operation `admit_removal` — admit one removal plan into affine removal
/// authority. A grant is not an admission — this boundary mints the ladder's
/// own affine authority.
...
/// Operation `commit_removal` — cross the destructive boundary exactly once,
/// consuming the admission. Establishes: the `RemovalCommitment` fact and
/// the completed-ladder `RemovalReceipt`. Authorized removal is never
/// historical absence.
